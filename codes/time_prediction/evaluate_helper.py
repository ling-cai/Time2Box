...
def get_thresholds(valid_time_scores_info, test_time_scores_info, aggr='mean', verbose=False):
...
def compute_scores(id_year_map, time_scores_info,  method=None, score_func=None,
    ...
    for ele in time_scores_info: # for each triple,[<s, r, o, start, end>, scores, rank]
        scores_t.append(ele[-1])
        
        ele = ele[0]
        t1 = ele[3]
        t2 = ele[4]

        t_gold_min.append(t1)
        t_gold_max.append(t2)
        gold_durations.append(t2 - t1)

    gold_durations = torch.tensor(gold_durations)
    t_gold_min = torch.tensor(t_gold_min)
    t_gold_max = torch.tensor(t_gold_max)
    scores_t = torch.stack(scores_t).squeeze(1)
    print('shape of scores_t', scores_t.shape)
    ...
    t_pred_min, t_pred_max = (pred_min.float(), pred_max.float())
    ...
    return iou_scores, t_gold_min, t_gold_max, t_pred_min, t_pred_max


def compute_preds(id_year_map, time_scores_info, method=None, score_func=None, thresholds=None,
