...
def gaeiou_score(pred_start, pred_end, gold_start, gold_end, delta=0):
	'''
	computes aeiou score (affinity enhanced iou) defined as max{1, overlap_time}/(minimum span that covers both)
	params-
	pred_start: [n x 1] tensor, predicted start time
	...
	#--IMPORTANT!- as (t,t) interval length is counted 0---#
	loc = torch.nonzero(numerator.ge(0), as_tuple=True)
	mask = torch.zeros_like(numerator)
	mask[loc] = 1
	numerator = mask*(numerator+1) + (1-mask)*1./(torch.abs(numerator)+1)
	#numerator+=1
	denomerator+=1
	#--------------#
